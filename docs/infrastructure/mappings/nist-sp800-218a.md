...
| **PS.1.3** Protect model weights and configuration *(new task)* | High | Directly relevant for self-hosted open-weight deployments. Deployers running their own models must implement weight protection controls. | DP-01 through DP-08 (Data Protection), see also [Open-Weight Models Shift the Burden](../../core/open-weight-models-shift-the-burden.md) |
